module shtns_backend_mod
   ...
   use iso_c_binding, only: c_int, c_double, c_double_complex, c_ptr, c_null_ptr, c_associated, c_f_pointer
...
contains
   ...
   subroutine shtns_init(state, nlon, nlat, ntrunc)
      include 'external/shtns/shtns.f03'
      ...
      state%cfg = shtns_create(int(ntrunc, c_int), int(ntrunc, c_int), int(1, c_int), norm)
      ...
      eps_polar = 1.0d-10
      call shtns_set_grid(state%cfg, layout, eps_polar, int(nlat, c_int), int(nlon, c_int))

      call c_f_pointer(state%cfg, info)
      state%nlm = info%nlm
      state%nlat = nlat
      state%nphi = nlon
      state%ntrunc = ntrunc
   ...
   subroutine shtns_configure(state, nthreads)
      ...
      if (nthreads > 0) then
         configured = shtns_use_threads(int(nthreads, c_int))
      endif
   ...
   subroutine shtns_destroy(state)
      ...
      if (c_associated(state%cfg)) then
         call shtns_unset_grid(state%cfg)
         call shtns_destroy(state%cfg)
      endif
   ...
   subroutine shtns_spat2spec(z_lat_phi, u_l_m, state)
      ...
      call spat_to_sh(state%cfg, state%spat_phi_lat, state%qlm)

      u_l_m = (0.0, 0.0)
      do m = 0, state%ntrunc
         do l = m, state%ntrunc
            lm = shtns_lmidx(state%cfg, int(l, c_int), int(m, c_int))
   ...
   subroutine shtns_spec2spat(z_lat_phi, u_l_m, state)
      ...
      call sh_to_spat(state%cfg, state%qlm, state%spat_phi_lat)
