...
!   integer :: nmelt, nfiles, iter, itersl, dtime           ! Number of melting episodes up to the current timestep
   ...
   subroutine sl_solver_checkpoint(itersl, dtime)
      ...
      if (shtns_eps < 0.0) then
         write(unit_num,*) 'SHTns eps must be >= 0; resetting to 1.0e-10.'
         shtns_eps = 1.0e-10
      endif
      ...
      call sh_initialize(spheredat, sh_backend, 2*nglv, nglv, norder, radius, unit_num, &
                         ducc_direct_map, ducc_sht_threads, shtns_sht_threads, &
                         shtns_eps, shtns_allow_padding, shtns_grid_type)
      ...
      elseif (backend_key == 'shtns' .or. backend_key == 'SHTNS' .or. backend_key == 'Shtns') then
         write(unit_num,'(A,A,A,I0,A,ES10.3E2,A,L1,A,A)') 'SH backend config: backend=', trim(sh_backend), &
              ' shtns_sht_threads=', shtns_sht_threads, ' shtns_eps=', shtns_eps, &
              ' shtns_allow_padding=', shtns_allow_padding, ' shtns_grid_type=', trim(shtns_grid_type)
      else
